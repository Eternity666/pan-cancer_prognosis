...
def data_generator(cancers, train_cancers):
    ...
    for cancer_type in cancers:
        ...
        modality_index['cna'] = [single_clinical_data.shape[1],
                                  single_clinical_data.shape[1] + single_cna.shape[0]]

        single_cna = single_cna.T  # 一列是一个cna
        single_data = pd.merge(single_clinical_data, single_cna,
                               left_index=True, right_index=True, how='left')
        ...
        cutoff = 0.8
        num_to_retain = int((1 - cutoff) * single_cna.shape[1])  # 保留至少有 num_to_retain 个非空的行
        ...
        single_data = pd.concat([single_clinical_data, single_cna], axis=0)
    ...
    cna = all_data.iloc[modality_index['clin'][1]:, :]
    ...
    all_data = pd.concat([clinical_data, cna], axis=0)
    modality_index['clin'] = [2, clinical_data.shape[0]]
    modality_index['cna'] = [clinical_data.shape[0], all_data.shape[0]]
    ...
    print(f"Total number of cna retained: {modality_index['cna'][1] - modality_index['cna'][0]}")
...
class CancerDataset:
    def __init__(self, cna, clin, survival, status):
        self.cna = cna
        self.clin = clin
...
def train_func(model, optimizer, loss_func, data_loader, device):
    ...
    for data in data_loader:
        ...
        #     continue
        survival_output = model(input_cna, input_clin)
...
class Network(torch.nn.Module):
    ...
    def __init__(self, num_cna, num_clin):
    ...
    def forward(self, cna, clin):
        ...
        hadamard = torch.mul(x, y)

        fusion = torch.cat((x, y, mean, hadamard), dim=1)
...
def run_training(x_train, clin_train, y_train, status_train,
    ...
    network = Network(num_cna=x_train.shape[1], num_clin=clin_train.shape[1])
    early_stopping = EarlyStopping(patience=20, verbose=True,
                                   path='/mnt/yzh/pancancer/code/cna+clin_network/model/BEST_MODEL.pth')
    ...
    network = Network(num_cna=x_test.shape[1], num_clin=clin_test.shape[1])
    network.load_state_dict(torch.load(f"/mnt/yzh/pancancer/code/cna+clin_network/model/BEST_MODEL.pth"))
    network.to(DEVICE)
    ...
    plt.savefig(f"/mnt/yzh/pancancer/code/cna+clin_network/plot/cna+clin.jpg", format='jpg')
    plt.close()
...
def train_val_test(train_valid_data, test_data, independent_data,
    ...
    for cv_round in range(FOLDS):
        ...
        cna_train, clin_train, y_train, status_train = sort_data(cna_train, clin_train, y_train, status_train)
        ...
        cna_valid, clin_valid, y_valid, status_valid = sort_data(cna_valid, clin_valid, y_valid, status_valid)

        cna_feature_train = cna_train
        cna_feature_valid = cna_valid
        cna_feature_test = cna_test
        cna_feature_independent = cna_independent
        ...
        selector = SelectKBest(score_func=f_regression,
                               k=NUM_CNA_FEATURES).fit(cna_feature_train[status_train, :], y_train[status_train])
        # p_values = selector.pvalues_
        print("Feature Selection Algorithms: f_regression")
        cna_feature_train = selector.transform(cna_feature_train)
        cna_feature_valid = selector.transform(cna_feature_valid)
        print("Num of features selected:", NUM_CNA_FEATURES)
        print("Done!\n")

        cna_feature_test = selector.transform(cna_feature_test)
        cna_feature_independent = selector.transform(cna_feature_independent)

        print("Start to build Deep Neural Network...")
        valid_c_index, test_c_index, independent_c_index = \
            run_training(cna_feature_train, clin_train, y_train, status_train,
                         cna_feature_valid, clin_valid, y_valid, status_valid,
                         cna_feature_test, clin_test, y_test, status_test,
                         cna_feature_independent, clin_independent, y_independent, status_independent
                         )
