...
NUM_CNA_FEATURES = 3000


def sort_data(x, y, z, w, t, e):
...
def data_generator(cancers, train_cancers):
    ...
    modalities = ['clin', 'gene', 'cna', 'micro']
    ...
    for cancer_type in cancers:
        ...
        single_cna_path = f"/mnt/yzh/pancancer/cancer_dataset/{cancer_type}/data_CNA.txt"
        single_microbiome_path = f"/mnt/yzh/pancancer/cancer_dataset/{cancer_type}/data_microbiome.txt"
        ...
        single_cna = pd.read_table(single_cna_path, index_col=0, low_memory=False)
        ...
        single_cna.drop(columns=['Entrez_Gene_Id'], inplace=True)
        single_cna.drop(index=['MARCH1', 'MARCH2', 'SEPT15', '1-Mar', '2-Mar'], errors='ignore', inplace=True)
        single_cna.columns = single_cna.columns.map(lambda x: x[:-3])
        ...
        single_cna = single_cna.T  # 一列是一个cna
        ...
        single_cna = single_data.iloc[modality_index['cna'][0]: modality_index['cna'][1], :]
        ...
        single_cna = single_cna.dropna(thresh=num_to_retain)
        ...
        for index in list(single_cna.index[single_cna.isnull().sum(axis=1) > 0]):
            mean_val = single_cna.loc[index, :].mean(skipna=True)
            single_cna.loc[index, :].fillna(mean_val, inplace=True)
        ...
        single_data = pd.concat([single_clinical_data, single_gene_expression, single_cna, single_microbiome], axis=0)
        ...
        if cancer_type == cancers[0]:
            ...
            cna = pd.concat([cna, single_cna], axis=1, join='outer')
        else:
            gene_expression = pd.concat([gene_expression, single_gene_expression], axis=1, join='inner')
            cna = pd.concat([cna, single_cna], axis=1, join='inner')
    ...
    gene_cna_micro = all_data.iloc[modality_index['clin'][1]:, :]
    # gene_expression = all_data.iloc[modality_index['clin'][1]: modality_index['gene'][1], :]
    # cna = all_data.iloc[modality_index['gene'][1]:, :]
    ...
    all_data = pd.concat([clinical_data, gene_cna_micro], axis=0)
    ...
    modality_index['cna'] = [modality_index['gene'][1], modality_index['gene'][1] + cna.shape[0]]
    modality_index['micro'] = [modality_index['cna'][1], all_data.shape[0]]
    ...
    print(f"Total number of CNA features: {modality_index['cna'][1] - modality_index['cna'][0]}")
...
class CancerDataset:
    def __init__(self, gene, clin, cna, micro, survival, status):
        self.gene = gene
        self.clin = clin
        self.cna = cna
        self.micro = micro
    ...
    def __getitem__(self, idx):
        dct = {
            ...
            'cna': torch.tensor(self.cna[idx, :], dtype=torch.float),
...
class Network(torch.nn.Module):
    ...
    def __init__(self, num_gene, num_clin, num_cna, num_micro):
        ...
        self.fc_cna = nn.Linear(num_cna, 512)
        self.resNet_cna = ResNet(512, 6)
    ...
    def forward(self, gene, clin, cna, micro):
        x = self.fc_gene(gene)
        ...
        mean = torch.mean(torch.stack([x, y, z, w]), dim=0)
        hadamard = x * y * z * w

        fusion = torch.cat((x, y, z, w, mean, hadamard), dim=1)
...
def run_training(x_train, clin_train, cna_train, micro_train, y_train, status_train,
                 x_valid, clin_valid, cna_valid, micro_valid, y_valid, status_valid,
                 x_test, clin_test, cna_test, micro_test, y_test, status_test,
                 x_independent, clin_independent, cna_independent, micro_independent, y_independent, status_independent):

    train_dataset = CancerDataset(x_train, clin_train, cna_train, micro_train, y_train, status_train)
    valid_dataset = CancerDataset(x_valid, clin_valid, cna_valid, micro_valid, y_valid, status_valid)
    test_dataset = CancerDataset(x_test, clin_test, cna_test, micro_test, y_test, status_test)
    independent_dataset = CancerDataset(x_independent, clin_independent, cna_independent,
                                        micro_independent, y_independent, status_independent)
    ...
    network = Network(num_gene=x_train.shape[1], num_clin=clin_train.shape[1],
                      num_cna=cna_train.shape[1], num_micro=micro_train.shape[1])
    early_stopping = EarlyStopping(patience=20, verbose=True,
                                   path='/mnt/yzh/pancancer/code/gene+clin+cna+micro_network/model/BEST_MODEL.pth')
    ...
    network = Network(num_gene=x_test.shape[1], num_clin=clin_test.shape[1],
                      num_cna=cna_test.shape[1], num_micro=micro_test.shape[1])
    network.load_state_dict(torch.load(f"/mnt/yzh/pancancer/code/gene+clin+cna+micro_network/model/BEST_MODEL.pth"))
    network.to(DEVICE)
    ...
    # plt.title("C-index")
    plt.plot(epochs, valid_c_indexes, color='royalblue')
    ...
    plt.savefig(f"/mnt/yzh/pancancer/code/gene+clin+cna+micro_network/plot/gene+clin+cna.jpg", format='jpg')
    plt.close()
...
def train_val_test(train_valid_data, test_data, independent_data,
    ...
    cna_test = test_data[:, modality_index['cna'][0]: modality_index['cna'][1]]
    ...
    cna_independent = independent_data[:, modality_index['cna'][0]: modality_index['cna'][1]]
    ...
    for cv_round in range(FOLDS):
        ...
        cna_train = train_data[:, modality_index['cna'][0]: modality_index['cna'][1]]
        micro_train = train_data[:, modality_index['micro'][0]: modality_index['micro'][1]]
        y_train = train_data[:, 0]
        status_train = train_data[:, 1].astype(np.bool)
        gene_train, clin_train, cna_train, micro_train, y_train, status_train = \
            sort_data(gene_train, clin_train, cna_train, micro_train, y_train, status_train)
        ...
        cna_valid = valid_data[:, modality_index['cna'][0]: modality_index['cna'][1]]
        micro_valid = valid_data[:, modality_index['micro'][0]: modality_index['micro'][1]]
        y_valid = valid_data[:, 0]
        status_valid = valid_data[:, 1].astype(np.bool)
        gene_valid, clin_valid, cna_valid, micro_valid, y_valid, status_valid = \
            sort_data(gene_valid, clin_valid, cna_valid, micro_valid, y_valid, status_valid)
        ...
        selector_cna = SelectKBest(score_func=f_regression,
                                   k=NUM_CNA_FEATURES).fit(cna_train[status_train, :], y_train[status_train])
        print("Feature Selection Algorithms of CNA: f_regression")
        cna_feature_train = selector_cna.transform(cna_train)
        cna_feature_valid = selector_cna.transform(cna_valid)
        cna_feature_test = selector_cna.transform(cna_test)
        cna_feature_independent = selector_cna.transform(cna_independent)
        print("Num of CNA features selected:", cna_feature_train.shape[1])
        print("Done!\n")

        print("Start to build Deep Neural Network...")
        valid_c_index, test_c_index, independent_c_index = \
            run_training(gene_feature_train, clin_train, cna_feature_train, micro_feature_train, y_train, status_train,
                         gene_feature_valid, clin_valid, cna_feature_valid, micro_feature_valid, y_valid, status_valid,
                         gene_feature_test, clin_test, cna_feature_test, micro_feature_test, y_test, status_test,
                         gene_feature_independent, clin_independent, cna_feature_independent, micro_feature_independent, y_independent, status_independent
                         )
