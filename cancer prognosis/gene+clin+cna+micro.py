...
def data_generator(cancers, train_cancers):
    ...
    for cancer_type in cancers:
        ...
        single_cna_path = f"F:\\cancer dataset\\{cancer_type}\\data_CNA.txt"
...
def run_training(cv_round, x_train, clin_train, cna_train, micro_train, y_train, status_train,
    ...
    # plt.title("C-index")
    plt.ylim((0, 1))
    plt.plot(epochs, valid_c_indexes, color='aqua')
    ...
    plt.savefig(f"F:\\plot\\gene+clin+cna_FOLD{cv_round}.jpg", format='png')
    plt.close()
...
def train_val_test(train_valid_data, test_data, independent_data,
    ...
    for cv_round in range(FOLDS):
        ...
        valid_c_index, test_c_index, independent_c_index = \
            run_training(cv_round, gene_feature_train, clin_train, cna_feature_train, micro_feature_train, y_train, status_train,
