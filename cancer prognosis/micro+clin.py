...
def data_generator(cancers, train_cancers):
    ...
    for cancer_type in cancers:
        ...
        modality_index['micro'] = [single_clinical_data.shape[1],
                                  single_clinical_data.shape[1] + single_microbiome.shape[0]]

        single_microbiome = single_microbiome.T  # 一列是一个microbiome
        single_data = pd.merge(single_clinical_data, single_microbiome,
                               left_index=True, right_index=True, how='left')
        ...
        cutoff = 0.8
        num_to_retain = int((1 - cutoff) * single_microbiome.shape[1])  # 保留至少有 num_to_retain 个非空的行
        ...
        single_data = pd.concat([single_clinical_data, single_microbiome], axis=0)
    ...
    microbiome = all_data.iloc[modality_index['clin'][1]:, :]
    ...
    all_data = pd.concat([clinical_data, microbiome], axis=0)
    modality_index['clin'] = [2, clinical_data.shape[0]]
    modality_index['microbiome'] = [clinical_data.shape[0], all_data.shape[0]]
    ...
    print(f"Total number of microbiome retained: {modality_index['micro'][1] - modality_index['micro'][0]}")
...
class CancerDataset:
    def __init__(self, micro, clin, survival, status):
        self.micro = micro
        self.clin = clin
...
def train_func(model, optimizer, loss_func, data_loader, device):
    ...
    for data in data_loader:
        ...
        #     continue
        survival_output = model(input_micro, input_clin)
...
class Network(torch.nn.Module):
    ...
    def forward(self, micro, clin):
        # 最简单版本的DNN，下次尝试 batch norm —— 若使用了，则在训练前要model.train()，测试时要model.eval()

        x = self.fc_micro(micro)
        x = self.resNet_micro(x)
...
def run_training(x_train, clin_train, y_train, status_train,
    ...
    network = Network(num_micro=x_train.shape[1], num_clin=clin_train.shape[1])
    early_stopping = EarlyStopping(patience=20, verbose=True,
                                   path='/mnt/yzh/pancancer/code/micro+clin_network/model/BEST_MODEL.pth')
    ...
    network = Network(num_micro=x_test.shape[1], num_clin=clin_test.shape[1])
    network.load_state_dict(torch.load(f"/mnt/yzh/pancancer/code/micro+clin_network/model/BEST_MODEL.pth"))
    network.to(DEVICE)
    ...
    plt.savefig(f"/mnt/yzh/pancancer/code/micro+clin_network/plot/micro+clin.jpg", format='jpg')
    plt.close()
...
def train_val_test(train_valid_data, test_data, independent_data,
    ...
    for cv_round in range(FOLDS):
        ...
        micro_train, clin_train, y_train, status_train = sort_data(micro_train, clin_train, y_train, status_train)
        ...
        micro_valid, clin_valid, y_valid, status_valid = sort_data(micro_valid, clin_valid, y_valid, status_valid)
        ...
        valid_c_index, test_c_index, independent_c_index = \
            run_training(micro_feature_train, clin_train, y_train, status_train,
                         micro_feature_valid, clin_valid, y_valid, status_valid,
                         micro_feature_test, clin_test, y_test, status_test,
                         micro_feature_independent, clin_independent, y_independent, status_independent
                         )
