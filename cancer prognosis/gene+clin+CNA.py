...
def data_generator(cancers, train_cancers):
    ...
    for cancer_type in cancers:
        ...
        single_data = pd.concat([single_clinical_data, single_gene_expression, single_cna], axis=0)
    ...
    gene_cna = all_data.iloc[modality_index['clin'][1]:, :]
    ...
    all_data = pd.concat([clinical_data, gene_cna], axis=0)
    ...
    modality_index['cna'] = [modality_index['gene'][1], all_data.shape[0]]
...
class CancerDataset:
    def __init__(self, gene, clin, cna, survival, status):
        self.gene = gene
        self.clin = clin
        self.cna = cna
...
def train_func(model, optimizer, loss_func, data_loader, device):
    ...
    for data in data_loader:
        ...
        survival_output = model(input_gene, input_clin, input_cna)
...
class Network(torch.nn.Module):
    ...
    def __init__(self, num_gene, num_clin, num_cna):
    ...
    def forward(self, gene, clin, cna):
        x = self.fc_gene(gene)
        ...
        mean = torch.mean(torch.stack([x, y, z]), dim=0)
        hadamard = x * y * z

        fusion = torch.cat((x, y, z, mean, hadamard), dim=1)
...
def run_training(x_train, clin_train, cna_train, y_train, status_train,
                 x_valid, clin_valid, cna_valid, y_valid, status_valid,
                 x_test, clin_test, cna_test, y_test, status_test,
                 x_independent, clin_independent, cna_independent, y_independent, status_independent):

    train_dataset = CancerDataset(x_train, clin_train, cna_train, y_train, status_train)
    valid_dataset = CancerDataset(x_valid, clin_valid, cna_valid, y_valid, status_valid)
    test_dataset = CancerDataset(x_test, clin_test, cna_test, y_test, status_test)
    ...
    network = Network(num_gene=x_train.shape[1], num_clin=clin_train.shape[1], num_cna=cna_train.shape[1])
    ...
    # test
    print("\n----------Start testing----------")
    network = Network(num_gene=x_test.shape[1], num_clin=clin_test.shape[1], num_cna=cna_test.shape[1])
    ...
    plt.savefig(f"F:\\plot\\gene+clin+cna.jpg", format='jpg')
    plt.close()
...
def train_val_test(train_valid_data, test_data, independent_data,
    ...
    for cv_round in range(FOLDS):
        ...
        gene_train, clin_train, cna_train, y_train, status_train = \
            sort_data(gene_train, clin_train, cna_train, y_train, status_train)
        ...
        gene_valid, clin_valid, cna_valid, y_valid, status_valid = \
            sort_data(gene_valid, clin_valid, cna_valid, y_valid, status_valid)
        ...
        valid_c_index, test_c_index, independent_c_index = \
            run_training(gene_feature_train, clin_train, cna_feature_train, y_train, status_train,
                         gene_feature_valid, clin_valid, cna_feature_valid, y_valid, status_valid,
                         gene_feature_test, clin_test, cna_feature_test, y_test, status_test,
                         gene_feature_independent, clin_independent, cna_feature_independent, y_independent, status_independent
                         )
